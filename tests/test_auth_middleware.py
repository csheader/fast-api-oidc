...
class TestAuthMiddleware(IsolatedAsyncioTestCase):
    ...
    @patch('fast_api_jwt_middleware.auth_middleware.jwt.PyJWK')
    @patch('fast_api_jwt_middleware.auth_middleware.jwt.decode')
    @patch('fast_api_jwt_middleware.auth_middleware.jwt.get_unverified_header')
    @patch('fast_api_jwt_middleware.auth_middleware.requests.get')
    ...
    @patch('fast_api_jwt_middleware.auth_middleware.AuthMiddleware.decode_token')
    ...
    def test_init_logger_none(self):
        ...
        """
        # Mock all the things.
        with patch('fast_api_jwt_middleware.auth_middleware.requests.get') as mock_get:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = {
                'issuer': 'https://example.com',
